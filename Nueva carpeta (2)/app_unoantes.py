    ...
    with tab3:
        ...
        try:
            ...
            if notas_resultado.data and len(notas_resultado.data) > 0:
                ...
                with col1:
                    total_coins = df_notas['coins_incentivo'].sum() if 'coins_incentivo' in df_notas.columns else 0
                    st.metric("💰 Total Incentivo Coins", f"{int(total_coins):,}")
                
                with col2:
                    total_paypal = df_notas['paypal_incentivo'].sum() if 'paypal_incentivo' in df_notas.columns else 0
                    st.metric("💵 Total Incentivo PayPal", f"${float(total_paypal):,.2f}")
                
                with col3:
                    total_sueldo = df_notas['paypal_bruto'].sum() if 'paypal_bruto' in df_notas.columns else 0
                    st.metric("💼 Total Sueldo", f"${float(total_sueldo):,.2f}")
                ...
                columnas_notas = ['usuario', 'dias', 'duracion', 'diamantes', 'nivel', 
                                 'paypal_bruto', 'coins_incentivo', 'paypal_incentivo']
                ...
                nombres_notas = {
                    ...
                    'nivel': 'Nivel',
                    'paypal_bruto': 'Sueldo',
                    'coins_incentivo': 'Incentivo Coin',
                    'paypal_incentivo': 'Incentivo PayPal'
                }
